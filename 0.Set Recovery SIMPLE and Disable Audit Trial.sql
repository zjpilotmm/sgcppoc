ALTER DATABASE DBS_CreditPredix_VIT_PFME_P2_MAIN_1_2_3_1_20190129_Clean SET RECOVERY SIMPLE WITH NO_WAIT
GO
...
It will then disable the triggers
*/
...
--While there are triggers enabled
WHILE EXISTS
(
	SELECT AT.*
	FROM
	...
	(--get list of schema names, trigger names, table names that have trigger enabled
		Select
		...
		AND T.is_disabled = 0 --get only enabled triggers
	) ObjectInfo 
...
BEGIN
	...
	SET @SQLQuery = 'DISABLE TRIGGER ['+@TriggerName+'] ON '+@TableName
